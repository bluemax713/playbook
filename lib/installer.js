...
export async function install() {
  ...
  if (await fileExists(claudeMdDest)) {
    ...
    await fs.copyFile(path.join(PACKAGE_ROOT, 'templates', 'CLAUDE.md'), claudeMdDest);
  ...
  // --- Install statusline (won't overwrite — users customize it) ---
  ...
  if (await fileExists(statuslineDest)) {
    summary.skipped.push('statusline.sh (already exists)');
  } else {
    await fs.copyFile(
      path.join(PACKAGE_ROOT, 'extras', 'statusline', 'statusline.sh'),
      statuslineDest
    );
    await fs.chmod(statuslineDest, 0o755);
    summary.installed.push('statusline.sh');
  }
  ...
  // Copy templates/ into .playbook/templates/ (/start compares templates/CLAUDE.md on update)
  const templatesSource = path.join(PACKAGE_ROOT, 'templates');
  const playbookTemplatesDir = path.join(PLAYBOOK_DIR, 'templates');
  await fs.mkdir(playbookTemplatesDir, { recursive: true });
  for (const file of await fs.readdir(templatesSource)) {
    await fs.copyFile(
      path.join(templatesSource, file),
      path.join(playbookTemplatesDir, file)
    );
  }

  // Copy extras/statusline/ into .playbook/ (mirror for the update mechanism)
  const extrasStatuslineSource = path.join(PACKAGE_ROOT, 'extras', 'statusline');
  const playbookExtrasDir = path.join(PLAYBOOK_DIR, 'extras', 'statusline');
  await fs.mkdir(playbookExtrasDir, { recursive: true });
  for (const file of await fs.readdir(extrasStatuslineSource)) {
    await fs.copyFile(
      path.join(extrasStatuslineSource, file),
      path.join(playbookExtrasDir, file)
    );
  }
